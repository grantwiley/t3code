...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const customModelsForSelectedProvider = getCustomModelSlugsForProvider(settings, selectedProvider);
  ...
  const selectedModelOptionsForDispatch = useMemo(() => {
    ...
    if (selectedProvider === "pi") {
      const piOptions = supportsReasoningEffort && selectedEffort ? { thinkingLevel: selectedEffort } : {};
      return Object.keys(piOptions).length > 0 ? { pi: piOptions } : undefined;
    }
    return undefined;
  ...
  const onProviderModelSelect = useCallback(
    (provider: ProviderKind, model: ModelSlug) => {
      ...
      setComposerDraftModel(
        activeThread.id,
        resolveAppModelSelection(provider, getCustomModelSlugsForProvider(settings, provider), model),
      );
      scheduleComposerFocus();
    },
    [
